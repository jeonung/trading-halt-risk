...
이 프로젝트는 작전세력 등의 이상 거래로 인해 주식이 **거래정지**되는 기업을 사전에 예측하는 것을 목표로 합니다. 주가 및 거래량 등 시장 지표의 이상 움직임을 기술적 지표를 변수로 활용해 **거래정지 위험도**를 산출하고, 이를 통해 투자자들이 피해를 줄일 수 있도록 지원합니다.
...
- 매주 적재되는 데이터를 자동으로 모델에 반영하기 위해 `Windows 작업 스케줄러 + Python`을 활용하여 토요일 아침에 자동으로 업데이트되도록 자동화 프로세스 구축
